...
- Each `Child` has multiple `Observations` and can be in more than one `Organisation` via `UserOrganisation`
...
| UserRegistration   | oauth_server                    |                | We don't expect any ID collision between providers but just in case                                                                                                                                                                                                                    |
| UserRegistration   | id                              | -              | SHA-256 of `sub` from the login provider. Hash cannot be salted since we don't want to store `sub` anywhere in our database                                                                                                                                                            |
| UserRegistration   | salt/iterations                 | -              | Configuration for deriving the user encryption key                                                                                                                                                                                                                                     |
...
| Organisation       | default_reference               | -              | Which reference to use by default when creating new children under this organisation (e.g. uk-who for the NHS)                                                                                                                                                                         |
...
| Child              | encrypted_date_of_birth         | Child          | Required to call the dGC API. dGC key parameter - must regenerate all DGCResults if changed                                                                                                                                                                                            |
| Child              | sex                             | -              | Stored in plaintext so it can be used for analysis. dGC key parameter - must regenerate all DGCResults if changed.                                                                                                                                                                     |
| Child              | gestation_days                  | -              | Unlike the API that has weeks and days as separate parameters we just store cumulative days. Only set if known otherwise term is assumed when calling the API. Again stored in plaintext so it can be used for analysis. dGC key parameter - must regenerate all DGCResults if changed |
| Child              | reference                       | -              | The reference to be used when adding new observations - avoids having to select it every time. We don't delete DGCResults if the reference changes                                                                                                                                     |
| ChildOrganisation  | encrypted_child_key             | Organisation   |                                                                                                                                                                                                                                                                                        |
| Observation        | observation_type                | -              | `height \| weight \| ofc`                                                                                                                                                                                                                                                              |
| Observation        | encrypted_observation_date      | Child          | Encrypted out of an abundance of caution that correlating multiple observations could infer a date of birth                                                                                                                                                                            |
| Observation        | days_since_birth                | -              | Not an identifier (unlike date of birth or potentially observation date). Stored plaintext for analysis                                                                                                                                                                                |
| Observation        | observation_value               | -              | Decimal                                                                                                                                                                                                                                                                                |
| DGCResult          | reference                       | -              | The reference data used (e.g. uk-who, cdc, turners etc). Existing instances retained for analysis even if reference is changed                                                                                                                                                         |
| DGCResult          | encrypted_dgc_api_result        | Child          | Data to render the chart. Includes date of birth and observation date so must be encrypted.                                                                                                                                                                                            |
| DGCResult          | corrected/chronological_sds     | -              | Decimal. Stored as plaintext for analysis.                                                                                                                                                                                                                                             |
| DGCResult          | corrected/chronological_centile | -              | Integer 0-100. Stored as plaintext for analysis.                                                                                                                                                                                                                                       |
